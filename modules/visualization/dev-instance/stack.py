    ...
    def __init__(
        ...
        **kwargs,
        ...
        dcv_license_policy_json = {
            "Effect": "Allow",
            "Action": ["s3:GetObject"],
            "Resource": [f"arn:{self.partition}:s3:::dcv-license.{self.region}/*"],
        }

        s3_policy_json = {
            "Effect": "Allow",
            "Action": ["s3:Get*", "s3:List*", "s3:PutObject*", "s3:DeleteObject*"],
            "Resource": [f"arn:{self.partition}:s3:::{project_name}*", f"arn:{self.partition}:s3:::{project_name}*/*"],
        }

        lambda_policy_json = {
            "Effect": "Allow",
            "Action": ["lambda:Invoke*"],
            "Resource": [f"arn:{self.partition}:lambda:{self.region}:{self.account}:function:{project_name}-*"],
        }
        ...
        if s3_dataset_bucket:
            public_aev_dataset_policy_json = {
                ...
                "Resource": [
                    f"arn:{self.partition}:s3:::{s3_dataset_bucket}",
                    f"arn:{self.partition}:s3:::{s3_dataset_bucket}/*",
                ],
            }
